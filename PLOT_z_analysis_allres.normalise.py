...
def plot_data(z,res,i):
	...
	plt.plot(bin_mid/10,n/np.max(z),color=colors[i], label=res) #,linestyle='dashed')
...
ax.plot([-2,-2],[0,2],linewidth=15,color='gray',alpha=0.3)
ax.plot([2,2],[0,2],linewidth=15,color='gray',alpha=0.3)
plt.xlim(-2.5,2.5)
plt.legend(fontsize=15,frameon=False)
#plt.ylim(0,np.max(limits)+(0.1*np.max(limits)))
plt.ylim(0,1.1)
plt.savefig('Figs/Z_analysis_%s.all_res.norm.highres.pdf' % sys.argv[1],bbox_inches='tight', dpi=1800,format='pdf')
