...
def plot_data(z_cdl,z,res,i):
	n_cdl, bins, patches = plt.hist(z_cdl, bins=25,range=[-30,30], alpha=0)
	n, bins, patches = plt.hist(z, bins=25,range=[-30,30], alpha=0)
	print n
	print n_cdl
	print n_cdl/n
	bin_mid = (bins[:-1] + bins[1:]) / 2
	plt.plot(bin_mid/10,n_cdl/n,color=colors[i], label=res) #,linestyle='dashed')
...
for i in np.arange(1,20):
	try:
		...
		z_cdl = []
        	for pdb in pdbs:
			get_data(res,pdb)
			get_data_cdl(res,pdb)
		plot_data(z_cdl, z, res,i-1)
	except IndexError:
		pass

plt.ylabel('lipid contacts', fontsize=25 )
plt.xlabel('z-axis position (nm)', fontsize=25 )
ax.plot([-2,-2],[0,1],linewidth=15,color='gray',alpha=0.3)
ax.plot([2,2],[0,1],linewidth=15,color='gray',alpha=0.3)
plt.xlim(-2.5,2.5)
plt.legend(fontsize=15,frameon=False)
#plt.ylim(0,np.max(limits)+(0.1*np.max(limits)))
plt.ylim(0,1)
plt.savefig('Figs/Z_analysis_%s.cdl.normalise.highres.pdf' % sys.argv[1],bbox_inches='tight', dpi=1800,format='pdf')
